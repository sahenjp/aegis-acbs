...
func main() {
	...
	outPath := flag.String("output", "hierarchy-interleaved.txt", "output file")
	...
	pairs := make([]pair, 0, *queries*len(seeds))
	...
	fmt.Fprintln(w, "AEGIS_HIERARCHY_INTERLEAVED_V1")
	...
	fmt.Printf("hierarchy interleaved export: nodes=%d edges=%d queries=%d seeds=%v\n", len(g.Nodes), actualEdges, len(pairs), seeds)
}
