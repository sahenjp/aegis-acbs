...
func main() {
	...
	// Initialize reusable query buffers outside the measured loop.
	warmups := 3
	...
	for i, q := range sourceReport.QueryPairs {
		...
		for repeat := 0; repeat < *repeats; repeat++ {
			started := time.Now()
			cost, path = cg.ShortestPath(int64(q.Source), int64(q.Target))
		...
		results = append(results, queryResult{
			QueryIndex: i, Source: q.Source, Target: q.Target,
			DurationNS: duration, Distance: distance, Reachable: reachable, Correct: correct,
		})
	}
	...
	out := outputReport{
		Graph: g.Name, Nodes: len(g.Nodes), Edges: g.EdgeCount,
		Queries: len(sourceReport.QueryPairs), Repeats: *repeats,
		PreprocessNS: preprocessNS, Shortcuts: cg.GetShortcutsNum(),
		HeapAllocBytes: mem.Alloc, HeapSysBytes: mem.HeapSys,
		AllCorrect: allCorrect, Results: results,
	}
	...
	fmt.Printf("ch-baseline queries=%d preprocess=%.3fs median=%.3fms p95=%.3fms p99=%.3fms shortcuts=%d correct=%v\n",
		out.Queries, float64(out.PreprocessNS)/1e9, float64(out.MedianNS)/1e6,
		float64(out.P95NS)/1e6, float64(out.P99NS)/1e6, out.Shortcuts, out.AllCorrect)
	if !allCorrect {
		os.Exit(2)
	}
}
