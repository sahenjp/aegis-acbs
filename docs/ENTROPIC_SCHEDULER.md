...
進行率は桁単位で変動するため、加法的平均ではなく log 空間で更新します。
...
一方向への配分を構造的に排除するため、\(\rho=1/8\) の一様分布と混合します。
...
この設計は online convex optimization、UCB 型探索、entropy regularization、error-diffusion rounding を ACBS の証明進行制御へ組み合わせた engineering synthesis です。個々の数学要素の新規性を主張しません。production 比較では、内部処理を使わない同一探索基盤上の `aegis` を直接 baseline とし、CH・MLD・landmark 系ルーターとは別軸で評価します。
